            ...
            <div class="wrapper">
                ...
                <div class="container-fluid">
                    <div id="heading" class="page-header">
                        <h1><i class="icon20 i-table-2"></i>Auctions</h1>
                    </div>
                                ...
                                <div class="panel-body">
                                    ...
                                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered table-hover" id="dataTable">
                                        ...
                                        <tbody>
											...
											<?php 
											$auction=$this->admin_model->getAuction();
											if(!empty($auction)){
											foreach($auction as $value){ 
											?>
												<tr class="gradeX" id="<?= $value['id']?>">
                                                <td><?= $this->admin_model->findCustomerNameById($value['userId']); ?></td>
                                                <td><?= $value['request']; ?></td>
												<td><?php if($value['category'])
												echo $value['category'];
												else
												echo "no category";?></td>
                                                <td><?= $value['location']; ?></td>
                                               <td><?= $value['minBudget']; ?></td>
                                               <td><?= $value['maxBudget']; ?></td>
                                               <td><?= $value['dateOfService']; ?></td>
                                               <td><?= $value['noOfPeople']; ?></td>
                                               <td><?= $value['note']; ?></td>
                                               <td><?php
												$created=explode(" ",$value['createdOn']);
													echo $created[0];?></td>
                                                <td>
																									<a href="admin/edit_auction?id=<?= $value['id']?>"><i aria-hidden="true" class="i-marker-2" style="cursor:pointer;"></i></a>
																									<a ><i aria-hidden="true" class="i-remove-4 del_auction" title="Delete" style="cursor:pointer;"></i></a>
