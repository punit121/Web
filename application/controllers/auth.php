	...
	$plivo_hash=md5($plivo);
	   ...
	   function verify_otp($plivo)
	   {
		...
		session_start();*/
	   	...
	   	if($plivo == $otp_hash)
	   	{
	   		$update="UPDATE users SET activated='1' WHERE monum=$mobile";
	   		$this->db->query($update);
...
$this->email->subject('Password Request');
		$this->email->message('Hi there'. '<br/>'.
					'                                  '.'<br/>'.
					 ...
					 '                                  '.'<br/>'.
					'<a href="http://m.zersey.com/auth/request_password/'.$email.'">Click here </a> to reset your  password'."<br/>".
					'                                  '.'<br/>'.
