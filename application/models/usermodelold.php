...
function loginBygoogle($email)
	...
	function onlyServices($service,$start,$cnt)
	{
		...
		else
		{
			...
			$i=0;
			foreach($service as $sr){
				$a[$i]=" salonName like '%".$sr."%' ";
				$i++;
				}
				//print_r($service);
			$ps=implode("or",(array_unique($a)));
			$query=$this->db->query("select * from merchant where status='1' and (salonName like '%".$as."%' or $ps $mquery$aquery$cquery) LIMIT $start, $cnt ");
	...
	function serviceAndLocation($service,$location,$start,$cnt)
	{
		...
		*/	
		foreach($servic as $sr){
				$a[$i]=" ding.serviceName LIKE '%".$sr."%' or ding.keywords LIKE '%".$sr."%' or mrch.salonName LIKE '%".$sr."%' ";
				$i++;
				}
			$ps=implode("or",(array_unique($a)));
