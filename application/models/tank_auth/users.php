...
class Users extends CI_Model
{
	...
	{	
		...
		if ($this->db->insert($this->table_name, $data)) {
			...
			$merchantid=$this->userisMerchantOrNot($user_id);
			...
			{
				$customerid=$this->userisCustomerOrNot($user_id);
				$name=$customerid[0]['firstName'].' '.$customerid[0]['lastName'];
				if($customerid[0]['id']>0)
				{
					$add=array(
					'userId'=>$customerid[0]['id'],
					'mobile'=>$customerid[0]['monum'],
					'fullName'=>$name
					);
	...
	function userisCustomerOrNot($id)
	{
		$this->db->select('id,firstName,lastName,monum');
		$this->db->where('id',$id);
		$this->db->where('business','C');
		$query=$this->db->get('users');
		$data=$query->result_array();
		return($data)?$data:false;
	}
